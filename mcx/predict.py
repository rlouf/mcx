...
from mcx.compiler import compile_to_prior_sampler, compile_to_sampler, compile_to_posterior_sampler
...
class posterior_predictive:
    def __init__(self, rng_key: jax.random.PRNGKey, model, trace: Trace) -> None:
        ...
        artifact = compile_to_posterior_sampler(model.graph, model.namespace)
...
class prior_predictive:
    def __init__(self, rng_key: jax.random.PRNGKey, model) -> None:
        artifact = compile_to_prior_sampler(model.graph, model.namespace)
        sampler = jax.jit(artifact.compiled_fn)
...
def sample_forward(
    ...
    **observations
    ...
    artifact = compile_to_sampler(model.graph, model.namespace)
    sampler = jax.jit(artifact.compiled_fn)
